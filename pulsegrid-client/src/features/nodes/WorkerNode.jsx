...
import { WorkerGraphic } from '../../components/shared/NodeGraphics'; // <-- Change import per file
...
const WorkerNode = ({ id, data, selected }) => {
  ...
  const label = liveNode?.label || data.label || 'Worker';
  ...
  return (
    <div className={`relative group w-36 rounded-2xl bg-white/85 backdrop-blur-md p-3 border shadow-sm transition-all duration-300 ${colors.ring} ${selected ? 'ring-2 ring-indigo-500 shadow-xl' : ''}`}>
      ...
      <div className="w-full h-24 flex items-center justify-center">
        ...
        <WorkerGraphic colors={colors} className="w-24 h-24 drop-shadow-md transition-colors duration-500" />
      </div>
