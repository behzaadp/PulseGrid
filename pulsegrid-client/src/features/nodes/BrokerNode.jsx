...
const BrokerNode = ({ id, data, selected }) => {
  const liveNode = useEngineStore((state) => state.nodes[id]);
  const status = liveNode?.status || data.status || 'HEALTHY';
  const metrics = liveNode?.metrics || data.metrics || { cpu: 6, latency: 8, queueDepth: 0 };
  const label = liveNode?.label || data.label || 'Message Broker';
    ...
    <div className={`relative group w-44 rounded-2xl bg-white/85 backdrop-blur-md p-3.5 border transition-all duration-300 shadow-md hover:shadow-xl ${statusColors.ring} ${selected ? 'ring-2 ring-indigo-500 shadow-indigo-100' : ''}`}>
      ...
      <div className="w-full h-24 flex items-center justify-center">
        <svg viewBox="0 0 120 100" className="w-28 h-24 overflow-visible drop-shadow-sm">
          {/* Bottom Isometric Chassis */}
          <polygon points="20,52 60,72 100,52 60,32" fill="#E2E8F0" stroke="#CBD5E1" strokeWidth="1" />
          <polygon points="20,52 60,72 60,82 20,62" fill="#94A3B8" />
          <polygon points="60,72 100,52 100,62 60,82" fill="#64748B" />

          {/* Conveyor Channel Walls */}
          <path d="M 32,46 L 50,55 L 50,30 L 32,21 Z" fill="#F8FAFC" stroke="#E2E8F0" />
          <path d="M 50,55 L 88,36 L 88,11 L 50,30 Z" fill="#CBD5E1" />
          
          {/* Segmented Queue Slots (Message Envelopes) */}
          <polygon points="40,36 48,40 48,35 40,31" fill={statusColors.glow} fillOpacity="0.75" />
          <polygon points="54,33 62,29 62,24 54,28" fill={statusColors.glow} fillOpacity="0.9" />
          <polygon points="68,26 76,22 76,17 68,21" fill={statusColors.glow} fillOpacity="0.6" />

          {/* Ingress/Egress Conduit Rings */}
          <ellipse cx="32" cy="46" rx="5" ry="8" fill="#CBD5E1" stroke="#94A3B8" />
          <ellipse cx="88" cy="36" rx="5" ry="8" fill="#F8FAFC" stroke="#94A3B8" />
        </svg>
      </div>
      ...
      <div className="mt-2 grid grid-cols-3 gap-1 text-[10px] text-slate-600 font-mono">
        ...
        <div className="bg-slate-50/80 rounded px-1 py-0.5 border border-slate-100 text-center font-bold text-indigo-600">
          <span className="text-[8px] text-slate-400 block font-normal">DEPTH</span>
          {metrics.queueDepth}
        </div>
      </div>
    </div>
