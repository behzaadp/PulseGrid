  ...
  return (
    <div className={`absolute top-0 right-0 h-full w-80 bg-white/90 backdrop-blur-xl border-l border-slate-200 shadow-2xl transform transition-transform duration-300 z-20 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
        ...
        <button onClick={() => setSelectedNode(null)} className="text-slate-400 hover:text-slate-800 text-xl font-bold p-2">×</button>
      </div>
      ...
      <div className="p-5 flex-grow overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          <div className="p-3 border border-amber-200 bg-amber-50 rounded-xl">
            <div className="flex justify-between items-center mb-1">
              ...
              {node?.faults?.hasCpuSpike ? (
                <button onClick={() => clearFault('cpu_spike')} className="text-xs font-bold bg-amber-200 text-amber-800 px-2 py-1 rounded">Clear</button>
              ) : (
                <button onClick={() => triggerFault('cpu_spike')} className="text-xs font-bold bg-amber-500 text-white px-2 py-1 rounded hover:bg-amber-600">Inject</button>
              )}
            </div>
          ...
          {/* Memory Leak */}
          <div className="p-3 border border-rose-200 bg-rose-50 rounded-xl">
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm font-bold text-rose-900">Memory Leak</span>
              {node?.faults?.hasMemoryLeak ? (
                <button onClick={() => clearFault('memory_leak')} className="text-xs font-bold bg-rose-200 text-rose-800 px-2 py-1 rounded">Clear</button>
              ) : (
                <button onClick={() => triggerFault('memory_leak')} className="text-xs font-bold bg-rose-500 text-white px-2 py-1 rounded hover:bg-rose-600">Inject</button>
              )}
            </div>
        ...
        {/* Global Chaos (Danger Zone) */}
        <h3 className="text-xs font-bold text-rose-400 uppercase mt-8 mb-3">System-Wide Catastrophe</h3>
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => triggerScenario('thanos_snap')} className="bg-slate-800 text-white text-xs font-bold py-2 rounded shadow hover:bg-slate-700 border border-slate-900">
            Thanos Snap
          </button>
          <button onClick={() => triggerScenario('split_brain')} className="bg-slate-800 text-white text-xs font-bold py-2 rounded shadow hover:bg-slate-700 border border-slate-900">
            Split Brain
          </button>
        </div>
      </div>
